...
%
function [csdCath, csdAnod, StimCurrent_Cathodic, StimCurrent_Anodic] = Load_CSD_Electric_IO_Electrode_1(cfg)
...
for exps = 1:length(cfg.SSAnoexps)
    ...
    fprintf('%2d | %2d - %s | El. 1 | anodic',exps,length(cfg.SSAnoexps),cfg.SSAnoexps{exps})
    ...
    if strcmp(cfg.SSAnoexps{exps},'GP29E15CL') || strcmp(cfg.SSAnoexps{exps},'dummy')
        file = {sprintf('%sctx_pos%d_ss_sup_anodic0001.mat',pth,position), sprintf('%sctx_pos%d_ss_sup_anodic0002.mat',pth,position)};
        data(:,:,:,:) = Load_AO(file,15,'sweepBefore',50,'sweepAfter',200,'sweeps',32);
    else
        file = sprintf('%sctx_pos%d_ss_sup_anodic0001.mat',pth,position);
        data(:,:,:,:) = Load_AO(file,15,'sweepBefore',50,'sweepAfter',200,'sweeps',32);
    end
    ...
    for el = 1:size(data,1)
        csd(el,:,:) = calculate_csd(squeeze(mean(data(el,:,:,:),3)),0.15,1,1);
    end
    clear el data
    ...
    % Determine filenames according to experiment to load and load raw data
    if strcmp(cfg.SSAnoexps{exps},'GP29E15CL') || strcmp(cfg.SSAnoexps{exps},'dummy')
        file = {sprintf('%sctx_pos%d_ss_sup0001.mat',pth,position), sprintf('%sctx_pos%d_ss_sup0002.mat',pth,position)};
        data(:,:,:,:) = Load_AO(file,15,'sweepBefore',50,'sweepAfter',200,'sweeps',32);
    else
        file = sprintf('%sctx_pos%d_ss_sup0001.mat',pth,position);
        data(:,:,:,:) = Load_AO(file,15,'sweepBefore',50,'sweepAfter',200,'sweeps',32);
    end
    ...
    % Calculate current-source densities
    for el = 1:size(data,1)
        csd(el,:,:) = p_calculate_csd(squeeze(mean(data(el,:,:,:),3)),0.15,1,1);
    end
    clear el data
