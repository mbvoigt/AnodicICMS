%% Load CSD electric - electrode 16
%
...
%
function [csdCath, csdAnod, StimCurrent_Cathodic, StimCurrent_Anodic] = Load_CSD_Electric_IO_Electrode_16(cfg)
...
for exps = 1:length(cfg.SSAnoexps)
    ...
    fprintf('%2d | %2d - %s | El. 16 | anodic',exps,length(cfg.SSAnoexps),cfg.SSAnoexps{exps})
    ...
    if strcmp(cfg.SSAnoexps{exps},'GP29E15CL') || strcmp(cfg.SSAnoexps{exps},'dummy')
        file = {sprintf('%sctx_pos%d_ss_deep_anodic0001.mat',pth,position), sprintf('%sctx_pos%d_ss_deep_anodic0002.mat',pth,position)};
        data(:,:,:,:) = Load_AO(file,15,'sweepBefore',50,'sweepAfter',200,'sweeps',32);
    else
        file = sprintf('%sctx_pos%d_ss_deep_anodic0001.mat',pth,position);
        data(:,:,:,:) = Load_AO(file,15,'sweepBefore',50,'sweepAfter',200,'sweeps',32);
    end
    ...
    for el = 1:size(data,1)
        csd(el,:,:) = calculate_csd(squeeze(mean(data(el,:,:,:),3)),0.15,1,16);
    end
    clear el data
    ...
    % Determine filenames according to experiment to load and load raw data
    if strcmp(cfg.SSAnoexps{exps},'GP29E15CL') || strcmp(cfg.SSAnoexps{exps},'dummy')
        file = {sprintf('%sctx_pos%d_ss_deep0001.mat',pth,position), sprintf('%sctx_pos%d_ss_deep0002.mat',pth,position)};
        data(:,:,:,:) = Load_AO(file,15,'sweepBefore',50,'sweepAfter',200,'sweeps',32);
    else
        file = sprintf('%sctx_pos%d_ss_deep0001.mat',pth,position);
        data(:,:,:,:) = Load_AO(file,15,'sweepBefore',50,'sweepAfter',200,'sweeps',32);
    end
    ...
    % Calculate current-source densities
    for el = 1:size(data,1)
        csd(el,:,:) = p_calculate_csd(squeeze(mean(data(el,:,:,:),3)),0.15,1,16);
    end
    clear el data
